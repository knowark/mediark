...
def auth_provider() -> StandardAuthProvider:
    ...
    auth_provider = StandardAuthProvider(User(name="eecheverry"))
...
def test_standard_auth_validate_roles_correct_roles(auth_provider):
    ...
    # authenticated user
    auth_provider.state.user = User(name='john',  roles=['monitor'])
